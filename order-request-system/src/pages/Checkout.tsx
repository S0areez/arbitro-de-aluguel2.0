...
import { ArrowLeft, MapPin, CalendarDays, Clock, CreditCard, Smartphone, Wallet, Timer, TrendingUp } from "lucide-react";
...
const Checkout = () => {
 ...
 const handleConfirm = async () => {
  ...
  try {
    ...
    // 1. Criar a partida (Match) com tipagem explícita no 'from'
    const { data: matchCreated, error: matchError } = await supabase
      .from('matches')
      .insert({
        ...
        price: valorTotal,
        status: 'pendente', // Certifique-se que 'pendente' existe no seu ENUM do banco
        payment_method: pagamento,
        platform_fee: platformFee,
        is_surge: isSurge,
        duration: duration
      } as MatchInsert) // Cast para garantir compatibilidade
      .select()
      .single();

    if (matchError) throw matchError;
    
    // Solução para o erro "'matchCreated' is possibly 'null'"
    if (!matchCreated) {
      throw new Error("Erro ao retornar dados da partida criada.");
    }
    ...
    const { error: paymentError } = await supabase
      .from('payments')
      .insert({
        reserva_id: matchCreated.id, // Agora o TS sabe que matchCreated não é null
        user_id: user.id,
        status: 'pending',
        amount: valorTotal,
        external_reference: `match_${matchCreated.id}`
      } as PaymentInsert);

    if (paymentError) {
      console.error("Erro no log de pagamento:", paymentError);
    ...
    navigate(`/payment/${matchCreated.id}`);
      ...
      <div className="px-4 pt-4 pb-6 space-y-6">
        ...
        <div className="flex items-center gap-3 rounded-xl border border-border bg-card p-4">
          ...
          <div className="ml-auto text-right">
            <span className="block text-lg font-bold text-primary">R$ {valorTotal.toFixed(2)}</span>
        ...
        <Button 
          ...
          disabled={createMatch.isPending || isChecking} 
